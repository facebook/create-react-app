...
| LOCAL_WORKBOX        |          :x:           | :white_check_mark: | By default, the service worker created by Create React App will load the Workbox runtime libraries from Google Cloud Storage. When set to `true`, the script will use a local copy alongside the service worker. This is normally required when dealing with CSP or when it is prefered to host everything locally. |
